...
import { p as ie } from "./chunk-4BX2VUAB-CcLLIeQS.js";
import { p as se } from "./treemap-KMMF4GRG-8nUewU-W.js";
import { d as I } from "./arc-BRuxFFB1.js";
...
function ue() {
  ...
  function l(t) {
    var n,
      ...
      w = Math.min(F, Math.max(-F, s.apply(this, arguments) - v)),
      h,
      C = Math.min(Math.abs(w) / c, o.apply(this, arguments)),
      $ = C * (w < 0 ? -1 : 1),
      g;
    for (n = 0; n < c; ++n) (g = i[(u[n] = n)] = +e(t[n], n, t)) > 0 && (m += g);
    for (
      a != null
        ? u.sort(function (A, D) {
            return a(i[A], i[D]);
      ...
      ((d = u[n]),
        (g = i[d]),
        (h = v + (g > 0 ? g * x : 0) + $),
        (i[d] = { data: t[d], index: n, value: g, startAngle: v, endAngle: h, padAngle: C }));
    return i;
  }
  return (
    (l.value = function (t) {
...
var pe = ne.pie,
  N = { sections: new Map(), showData: !1 },
  T = N.sections,
  G = N.showData,
  de = structuredClone(pe),
  ge = p(() => structuredClone(de), "getConfig"),
  fe = p(() => {
    ((T = new Map()), (G = N.showData), re());
  }, "clear"),
  he = p(({ label: e, value: a }) => {
    if (a < 0)
      throw new Error(
...
` + e
    ...
    const w = l.textPosition,
      h = Math.min(x, d) / 2 - t,
      C = I().innerRadius(0).outerRadius(h),
      $ = I()
        .innerRadius(h * w)
        .outerRadius(h * w);
    u.append("circle")
    ...
    const O = [...g.entries()].map(([r, M]) => ({ label: r, value: M })),
      k = u
        .selectAll(".legend")
        .data(O)
        .enter()
  ...
  $e = { draw: Ce },
  Ge = { parser: xe, db: L, renderer: $e, styles: Ae };
